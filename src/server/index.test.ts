    ...
    describe('events', () => {
      ...
      describe('app_mention', () => {
        ...
        it('gets coronavirus update and splits it into several blocks', async () => {
          ...
          const githubScope = nock('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports')
            .get(`/${date}.csv`)
            .replyWithFile(200, resolve(__dirname, '..', '..', 'test', 'csv', '03-21-2020.csv'));
        ...
        it('gets coronavirus update and fits it in one block', async () => {
          ...
          const githubScope = nock('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports')
            .get(`/${date}.csv`)
            .replyWithFile(200, resolve(__dirname, '..', '..', 'test', 'csv', '03-20-2020.csv'));
          const slackScope = nock('https://slack.com/api')
            .post('/chat.postMessage', {
              channel: 'ABCDEF',
